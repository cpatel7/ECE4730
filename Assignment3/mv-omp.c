...
int main(int argc, char *argv[])
{
	...
	if (argc != 4)
	{
		printf("#usage: ./mv-serial input_matrix1 input_vector2 output_vector3 numCPU \n");
	}
	...
	start = clock();
	//multiply
	#pragma omp parallel for private(j,k,product)
	for (i=0; i<row1; i++)
	{
		for (j=0; j<col2; j++)
		{
	...
	printf("The parallel execution time is: %.2f\n\n",cpu_time_used);
	fprintf(speedup_file, "The parallel execution time for matrix of size %d and %d CPUs is: %.2f\n\n",row1,numCPU,cpu_time_used);
